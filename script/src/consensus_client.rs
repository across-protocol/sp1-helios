...
//             let fut: std::pin::Pin<Box<dyn Future<Output = Result<LightClientStore<S>>> + Send>> = async {
...
//         // ! todo: make this work. select_ok has to provide a way to drop &self reference.
...
//         //     Ok((store, futs)) => {
//         //         drop(futs);
//         //         // ! todo: we cannot assign here until we wait for all the futures to complete ? T_T
//         //         self.store = store;
//         //         Ok(())
//         //     },
//         //     Err(e) => Err(e),
...
//         // ! todo: for now, just wait for all futures to complete
//         let validated_results = join_all(futs).await;
//         match validated_results.into_iter().find_map(|result| {
//             match result {
//                 Ok(store) => Some(store),
//                 // todo? log error
//                 Err(_err) => None,
...
//                 Ok(())
//             },
//             None => Err(anyhow!("No rpc clients succeeded downloading bootstrap")),
//         }
//     }
...
//         let updates = Self::get_updates_single_rpc(expected_current_slot, current_finalized_slot, rpc).await;
...
//         helios_consensus_core::expected_current_slot(now, self.config.chain.genesis_time)
//     }
